...
const HostelRoomEdit = () => {
    ...
    const filteredHostelRoomList = hostelroomlist.filter(room =>
    ...
    const columns = [
        ...
        { key: 'no_of_bed', label: 'Number of Bed' },
    ...
    const fetchData = async () => {
        ...
        try {
            ...
            if (response && response.status && response.data) {
                ...
                // Find and populate the room being edited
                const roomToEdit = response.data.hostelroomlist?.find(room => room.id === id);
                if (roomToEdit) {
                    setFormData({
                        id: roomToEdit.id,
                        ...
                        no_of_bed: roomToEdit.no_of_bed,
                        cost_per_bed: roomToEdit.cost_per_bed,
                        description: roomToEdit.description || ''
                    });
                } else {
                    toast.error('Room not found');
                    navigate('/admin/hostelroom');
                }
            }
        } catch (error) {
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            ...
            if (response && response.status === true) {
                toast.success(response.message || 'Hostel room updated successfully');
                navigate('/admin/hostelroom');
            } else {
                toast.error(response.message || 'Failed to update hostel room');
            }
        } catch (error) {
    ...
    const handleDelete = async (roomId) => {
        if (window.confirm('Are you sure you want to delete this hostel room?')) {
            try {
                const response = await api.deleteHostelRoom(roomId);
