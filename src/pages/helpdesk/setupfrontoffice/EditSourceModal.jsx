...
const EditSourceModal = ({ show, onClose, onSuccess, sourceData }) => {
    ...
    const handleSubmit = async (e) => {
        ...
        setLoading(true);
        try {
            const response = await api.updateSource(sourceData.id, formData);
            toast.success(response.message || 'Source updated successfully');
            onSuccess();
        } catch (error) {
                ...
                <div className="modal-dialog" style={{ marginTop: '100px', maxWidth: '500px' }}>
                    <div className="modal-content" style={{ borderRadius: '12px', overflow: 'hidden', border: 'none', boxShadow: '0 20px 25px -5px rgba(0, 0, 0, 0.1)' }}>
                        <div className="modal-header" style={{ padding: '20px 24px', borderBottom: '1px solid #f1f5f9' }}>
                            ...
                            <h4 className="modal-title" style={{ fontWeight: '600', color: '#1e293b', fontSize: '18px' }}>Edit Source</h4>
                        </div>
