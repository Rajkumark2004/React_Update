...
const EditReferenceModal = ({ show, onClose, onSuccess, referenceData }) => {
    ...
    const handleSubmit = async (e) => {
        ...
        setLoading(true);
        try {
            const response = await api.updateReference(referenceData.id, formData);
                ...
                <div className="modal-dialog" style={{ marginTop: '100px', maxWidth: '500px' }}>
                    <div className="modal-content" style={{ borderRadius: '12px', overflow: 'hidden', border: 'none', boxShadow: '0 20px 25px -5px rgba(0, 0, 0, 0.1)' }}>
                        <div className="modal-header" style={{ padding: '20px 24px', borderBottom: '1px solid #f1f5f9' }}>
                            ...
                            <h4 className="modal-title" style={{ fontWeight: '600', color: '#1e293b', fontSize: '18px' }}>Edit Reference</h4>
                        </div>
                        <form onSubmit={handleSubmit}>
                            <div className="modal-body" style={{ padding: '24px' }}>
                                <div className="form-group" style={{ marginBottom: '20px' }}>
                                    <label style={{ color: '#475569', fontSize: '14px', fontWeight: '600', marginBottom: '8px', display: 'block' }}>Reference <small className="req" style={{ color: '#ef4444' }}>*</small></label>
                                    <input
